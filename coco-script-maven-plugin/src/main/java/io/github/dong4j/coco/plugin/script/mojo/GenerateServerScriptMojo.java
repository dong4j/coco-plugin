package io.github.dong4j.coco.plugin.script.mojo;
...
public class GenerateServerScriptMojo extends CocoAbstractMojo {

    /** Skip */
    @Parameter(property = Plugins.SKIP_LAUNCH_SCRIPT, defaultValue = Plugins.TURN_OFF_PLUGIN)
    private boolean skip;
    /** Output file */
    @Parameter(defaultValue = "${project.build.directory}/coco-plugin/bin/server.sh")
    ...
    public void execute() {
        if (this.skip) {
            this.getLog().info("coco-script-maven-plugin is skipped");
            return;
        }
